...
class MapModel extends Component {
  ...
  async retrieveData() {
    try {
      const STHML_PARK_Locations = await AsyncStorage.getItem(
      ...
      if (STHML_PARK_Locations !== null) {
        ...
        console.log("STHML_PARK_Locations2")
  ...
  async fetchParkinLocation(location) {
    ...
    const link =
      "https://openparking.stockholm.se/LTF-Tolken/v1/ptillaten/within?radius=" +
      radius +
      "&lat=" +
      latitude +
      "&lng=" +
      longitude +
      "&outputFormat=json&apiKey=f7e487cd-1921-46a3-aaa5-95e38127e6a2";
      console.log("STHML_PARK_Locations1")
  ...
  render() {
    ...
    let text2;
    this.getDistanceOneToOne(59.19858, 17.83317, 59.2000008, 17.8999996).then(
      allData => {
        text2 = allData;
      }
    );
